...
main( int argc, char* argv[] )
{
    ...
    try
    {
        ...
        const bool testCorrectness = Input("--correctness","test correctness?",true);
		...
		if( commRank == 0 )
        {
			...
			cout << "  Matrix size (m) = " << m << endl
				 << "  Algorithmic blocksize (nb) = " << nb << endl
				 << "  Local blocksize (nbLocal) = " << nbLocal << endl
				 << "  Grid Rows (r) = " << r << endl
				 << "  Grid Cols (c) = " << c << endl;
        }
        ...
        TestCholesky<double> ( testCorrectness, printMatrices, uplo, m, g );
