...
int main(int argc, char **argv) {
  ...
  // set all groups
  ie = MPI_Comm_group(MPI_COMM_WORLD, &group_world); checkErr(ie);
  ie = MPI_Group_range_incl(group_world, 1, range_garch, &group_garch); checkErr(ie);
  ie = MPI_Group_range_incl(group_world, 1, range_mat, &group_mat); checkErr(ie);
  
  // set communicators for each non-world group
  ie = MPI_Comm_create(MPI_COMM_WORLD, group_garch, &comm_garch); checkErr(ie);
  ie = MPI_Comm_create(MPI_COMM_WORLD, group_mat, &comm_mat); checkErr(ie);
